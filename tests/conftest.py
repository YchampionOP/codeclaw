"""Shared fixtures for codeclaw tests."""

import pytest

from codeclaw.anonymizer import Anonymizer
...
def mock_anonymizer(monkeypatch):
    ...
    monkeypatch.setattr(
        "codeclaw.anonymizer._detect_home_dir",
...
def tmp_config(tmp_path, monkeypatch):
    ...
    config_dir = tmp_path / ".codeclaw"
    config_file = config_dir / "config.json"
    monkeypatch.setattr("codeclaw.config.CONFIG_DIR", config_dir)
    monkeypatch.setattr("codeclaw.config.CONFIG_FILE", config_file)
    return config_file
